...
""")
    ...
    // BE-merged: managerial-position lookups for current/active managers
    @EntityGraph(attributePaths = {"person", "position", "department"})
    List<HrPersonPosition> findByCompanyIdAndPosition_IsManagerialTrueAndEndDateIsNull(Long companyId);

    @EntityGraph(attributePaths = {"person", "position", "department"})
    List<HrPersonPosition> findByCompanyIdAndPosition_IsManagerialTrueAndEndDateAfter(Long companyId, LocalDate today);
